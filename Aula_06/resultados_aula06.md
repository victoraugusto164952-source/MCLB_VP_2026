  TESTES CODIGO ORIGINAL

Teste 1 — Compra de imóvel

Frase inserida:
Quero comprar um apartamento de 3 quartos.

Resultado obtido:

Intenção: comprar_imovel

Confiança: 89.0%

Status da Decisão: IDENTIFICADO (comprar_imovel)

Resposta Automática do SAC:
**Atendimento de Vendas:** Ficamos felizes com seu interesse! Você pode conferir nosso catálogo de imóveis à venda em nosso site www.imobiliaria.com/vendas ou aguardar que um de nossos corretores entrará em contato em instantes.



Teste 2 — Aluguel de imóvel
Frase inserida:
Preciso alugar uma casa perto da faculdade.
Resultado obtido:
Intenção: alugar_imovel
Confiança: 65.1%
Status da Decisão: Atendimento de Locação.
Resposta Automática do SAC:
**Atendimento de Locação:** Temos ótimas opções disponíveis! Acesse www.imobiliaria.com/aluguel para filtrar por região e valor. Para agendar uma visita, envie o código do imóvel por aqui.

Teste 3 — Manutenção
Frase inserida:
O chuveiro do apartamento queimou e preciso de conserto.
Resultado obtido:
Intenção: suporte_manutencao
Confiança: 39.1%
Status da Decisão: IDENTIFICADO (suporte_manutencao)
Resposta Automática do SAC:
**Suporte e Manutenção:** Sentimos muito pelo inconveniente. Por favor, abra um chamado urgente em nosso portal do inquilino (www.imobiliaria.com/manutencao) anexando fotos ou vídeos do problema para acionarmos nossos prestadores.

Teste 4 — Boleto
Frase inserida:
Preciso da segunda via do boleto do aluguel.
Resultado obtido:
Intenção: 2via_boleto_contrato
Confiança: 55.4%
Status da Decisão:IDENTIFICADO (2via_boleto_contrato)
Resposta Automática do SAC:
**Financeiro e Contratos:** Para acessar boletos ou documentos, acesse a Área do Cliente em www.imobiliaria.com/cliente informando seu CPF e senha. Lá você baixa a 2ª via atualizada em segundos.



--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
LAB01

1 — Compra de imóvel
Frase inserida:
Quero comprar um apartamento de 3 quartos.
Resultado obtido:
Intenção: comprar_imovel
Confiança: 100.0%
Status da Decisão: IDENTIFICADO (comprar_imovel)
Resposta Automática do SAC:
**Atendimento de Vendas:** Ficamos felizes com seu interesse! Você pode conferir nosso catálogo de imóveis à venda em nosso site www.imobiliaria.com/vendas ou aguardar que um de nossos corretores entrará em contato em instantes.

2 — Aluguel de imóvel
Frase inserida:
Preciso alugar uma casa perto da faculdade.
Resultado obtido:
Intenção: alugar_imovel
Confiança: 100.0%
Status da Decisão: IDENTIFICADO (alugar_imovel)
Resposta Automática do SAC:
**Atendimento de Locação:** Temos ótimas opções disponíveis! Acesse www.imobiliaria.com/aluguel para filtrar por região e valor. Para agendar uma visita, envie o código do imóvel por aqui.

3 — Manutenção
Frase inserida:
O chuveiro do apartamento queimou e preciso de conserto.
Resultado obtido:
Intenção: suporte_manutencao
Confiança: 100.0%
Status da Decisão: IDENTIFICADO (suporte_manutencao)
Resposta Automática do SAC:
**Suporte e Manutenção:** Sentimos muito pelo inconveniente. Por favor, abra um chamado urgente em nosso portal do inquilino (www.imobiliaria.com/manutencao) anexando fotos ou vídeos do problema para acionarmos nossos prestadores.

4 — Boleto
Frase inserida:
Preciso da segunda via do boleto do aluguel.
Resultado obtido:
Intenção: 2via_boleto_contrato
Confiança: 100.0%
Status da Decisão:IDENTIFICADO (2via_boleto_contrato)
Resposta Automática do SAC:
**Financeiro e Contratos:** Para acessar boletos ou documentos, acesse a Área do Cliente em www.imobiliaria.com/cliente informando seu CPF e senha. Lá você baixa a 2ª via atualizada em segundos.



--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
LAB02

1 — Compra de imóvel
Frase inserida:
Quero comprar um apartamento de 3 quartos.
Resultado obtido:
Intenção: comprar_imovel
Confiança: 96.0%
Status da Decisão: IDENTIFICADO (comprar_imovel)
Resposta Automática do SAC:
**Atendimento de Vendas:** Ficamos felizes com seu interesse! Você pode conferir nosso catálogo de imóveis à venda em nosso site www.imobiliaria.com/vendas ou aguardar que um de nossos corretores entrará em contato em instantes.

2 — Aluguel de imóvel
Frase inserida:
Preciso alugar uma casa perto da faculdade.
Resultado obtido:
Intenção: comprar_imovel
Confiança: 43.9%
Status da Decisão:  UNCERTAIN (Fallback Acionado)
Resposta Automática do SAC:
Desculpe, não consegui compreender com clareza a sua solicitação. Estou transferindo agora mesmo sua conversa para um de nossos atendentes. Por favor, aguarde um momento.

3 — Manutenção
Frase inserida:
O chuveiro do apartamento queimou e preciso de conserto.
Resultado obtido:
Intenção: suporte_manutencao
Confiança: 84.1%
Status da Decisão: IDENTIFICADO (suporte_manutencao)
Resposta Automática do SAC:
**Suporte e Manutenção:** Sentimos muito pelo inconveniente. Por favor, abra um chamado urgente em nosso portal do inquilino (www.imobiliaria.com/manutencao) anexando fotos ou vídeos do problema para acionarmos nossos prestadores.

4 — Boleto
Frase inserida:
Preciso da segunda via do boleto do aluguel.
Resultado obtido:
Intenção: 2via_boleto_contrato
Confiança: 77.1%
Status da Decisão:IDENTIFICADO (2via_boleto_contrato)
Resposta Automática do SAC:
**Financeiro e Contratos:** Para acessar boletos ou documentos, acesse a Área do Cliente em www.imobiliaria.com/cliente informando seu CPF e senha. Lá você baixa a 2ª via atualizada em segundos.



-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
LAB03

1 — Compra de imóvel
Frase inserida:
Quero comprar um apartamento de 3 quartos.
Resultado obtido:
Intenção: comprar_imovel
Confiança: 89.0%
Status da Decisão: IDENTIFICADO (comprar_imovel)
Resposta Automática do SAC:
**Atendimento de Vendas:** Ficamos felizes com seu interesse! Você pode conferir nosso catálogo de imóveis à venda em nosso site www.imobiliaria.com/vendas ou aguardar que um de nossos corretores entrará em contato em instantes.

2 — Aluguel de imóvel
Frase inserida:
Preciso alugar uma casa perto da faculdade.
Resultado obtido:
Intenção: alugar_imovel
Confiança: 45.0%
Status da Decisão: UNCERTAIN (Fallback Acionado)
Resposta Automática do SAC:
Desculpe, não consegui compreender com clareza a sua solicitação. Estou transferindo agora mesmo sua conversa para um de nossos atendentes. Por favor, aguarde um momento.

3 — Manutenção
Frase inserida:
O chuveiro do apartamento queimou e preciso de conserto.
Resultado obtido:
Intenção: suporte_manutencao
Confiança: 76.0%
Status da Decisão: IDENTIFICADO (suporte_manutencao)
Resposta Automática do SAC:
**Suporte e Manutenção:** Sentimos muito pelo inconveniente. Por favor, abra um chamado urgente em nosso portal do inquilino (www.imobiliaria.com/manutencao) anexando fotos ou vídeos do problema para acionarmos nossos prestadores.

4 — Boleto
Frase inserida:
Preciso da segunda via do boleto do aluguel.
Resultado obtido:
Intenção: 2via_boleto_contrato
Confiança: 55.4%
Status da Decisão: UNCERTAIN (Fallback Acionado)
Resposta Automática do SAC:
Desculpe, não consegui compreender com clareza a sua solicitação. Estou transferindo agora mesmo sua conversa para um de nossos atendentes. Por favor, aguarde um momento.



---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
LAB04

1 — Cancelar aluguel
Frase inserida:
Quero cancelar meu contrato de aluguel.
Resultado obtido:
Intenção: cancelar_contrato
Confiança: 92.7%
Status da Decisão: IDENTIFICADO (cancelar_contrato)
Resposta Automática do SAC:
**Cancelamento de Contrato:** Para solicitar o cancelamento ou distrato, acesse a Área do Cliente em www.imobiliaria.com/cliente e envie sua solicitação. Nossa equipe de contratos analisará o pedido e informará os próximos passos.

2 — Cancelar contrato
Frase inserida:
Como faço para cancelar o contrato do imóvel?
Resultado obtido:
Intenção: cancelar_contrato
Confiança: 92.7%
Status da Decisão: IDENTIFICADO (cancelar_contrato)
Resposta Automática do SAC:
**Cancelamento de Contrato:** Para solicitar o cancelamento ou distrato, acesse a Área do Cliente em www.imobiliaria.com/cliente e envie sua solicitação. Nossa equipe de contratos analisará o pedido e informará os próximos passos.

3 — Como Cancelar
Frase inserida:
como faco pra cancelar meu contrato
Resultado obtido:
Intenção: cancelar_contrato
Confiança: 38.8%
Status da Decisão: UNCERTAIN (Fallback Acionado)
Resposta Automática do SAC:
Desculpe, não consegui compreender com clareza a sua solicitação. Estou transferindo agora mesmo sua conversa para um de nossos atendentes. Por favor, aguarde um momento.

4 — cancelamento
Frase inserida:
preciso falar sobre cancelamento.
Resultado obtido:
Intenção: suporte_manutencao
Confiança: 69.5%
Status da Decisão: IDENTIFICADO (suporte_manutencao)
Resposta Automática do SAC:
**Suporte e Manutenção:** Sentimos muito pelo inconveniente. Por favor, abra um chamado urgente em nosso portal do inquilino (www.imobiliaria.com/manutencao) anexando fotos ou vídeos do problema para acionarmos nossos prestadores.




